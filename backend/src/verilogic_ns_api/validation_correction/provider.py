...
from verilogic_ns_api.validation_correction.models import TaskKind


class CorrectionTaskRequest:
    def __init__(
        self,
        *,
        task_kind: TaskKind,
        ...
        self.namespace = (
            "semantic-critic.v1"
            if task_kind in {TaskKind.CRITIC_THEORY, TaskKind.CRITIC_QUERY}
            else "semantic-correction.v1"
        )
...
class OllamaCorrectionProvider:
    def __init__(
        ...
        *,
        ...
        if config.endpoint not in LOOPBACK_ENDPOINTS:
            raise ParserConfigurationError("correction endpoint must be loopback")
        if "cloud" in config.model.lower():
            raise ParserConfigurationError("cloud Ollama models are forbidden")
        self.config = config
        self._client = httpx.Client(
            ...
            transport=transport,
            trust_env=False,
            headers={"User-Agent": "VeriLogic-NS/0.1 validation-correction"},
        )
    ...
    def _request(self, method: str, path: str, **kwargs: object) -> object:
        try:
            ...
            raise ParserTimeoutError("local Ollama correction request timed out") from error
        except httpx.NetworkError as error:
            raise ParserTransientError("local Ollama correction transport failed") from error
        if response.status_code >= 500:
            raise ParserTransientError("local Ollama returned a temporary server error")
        if response.status_code >= 400:
            raise ParserConfigurationError(
                f"local Ollama rejected the correction request with status {response.status_code}"
            )
        try:
    ...
    def complete(self, request: CorrectionTaskRequest) -> ParserResponse:
        if request.config != self.config:
            raise ParserConfigurationError("correction request runtime differs from provider")
        ...
        try:
            ...
            raise ParserStructuredOutputError(
                "Ollama correction response violated its schema"
            ) from error
        if not response.done or response.model != self.config.model:
            raise ParserStructuredOutputError("Ollama returned an incomplete correction response")
        if response.message.thinking:
            raise ParserStructuredOutputError("thinking content was emitted despite think=false")
        try:
            content = json.loads(response.message.content)
        except json.JSONDecodeError as error:
            raise ParserStructuredOutputError("correction output was not JSON") from error
        if not isinstance(content, dict):
            raise ParserStructuredOutputError("correction output must be a JSON object")
